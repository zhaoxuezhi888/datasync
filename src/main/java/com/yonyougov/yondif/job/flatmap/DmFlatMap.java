...
public class DmFlatMap extends MessageFlatMap {
    private static final Logger logger = LogManager.getLogger(DmFlatMap.class);
    ...
    public Message flatMapAfter(Message message) {
        ...
        //如果是达梦,删除转为更新
        if (message.getDmlType().equals(DmlType.DELETE)) {
            message.setDmlType(DmlType.UPDATE);
            body.put("is_deleted", 2);
        }
        //字段映射转换,过滤特殊字符
